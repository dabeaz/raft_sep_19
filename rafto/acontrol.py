...
class AsyncRaftControl:
    ...
    async def _receiver(self):
        ...
        '''
        while True:
            msg = await self.net.recv()
            await self.events.put(('message', pickle.loads(msg)))
    ...
    async def _leader_heartbeat(self):
        ...
        '''
        while True:
            await curio.sleep(config.LEADER_HEARTBEAT)
            await self.events.put(('heartbeat', None))
    ...
    async def _election_timeout(self):
        ...
        while True:
            self._alive = False
            await curio.sleep(config.ELECTION_TIMEOUT_BASE
